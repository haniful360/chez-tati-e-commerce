...
const WishlistProducts = () => {
  ...
  const [loadingProductId, setLoadingProductId] = useState(null); // Loading state for each product

  const handleWishlistToggle = (product) => {
    setLoadingProductId(product.id); // Set the product id to indicate loading state
    const isProductInWishlist = wishlist.some((item) => item.id === product.id);

    // Toggle wishlist after a short delay (simulate async operation)
    setTimeout(() => {
  ...
  if (wishlist.length === 0) {
    return (
      <div className="max-w-[1320px] mx-auto  mt-40">
         <h2 className="text-sm md:text-xl lg:text-4xl font-semibold">Favorite</h2>
        <div className="flex flex-col items-center justify-center  min-h-screen pb-16 text-center">
          <EmptyIcon />
          <h2 className="text-lg font-semibold">You don't have any favorite products</h2>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-[1320px] mx-auto mt-40 pb-16">
      <h2 className="text-sm md:text-xl lg:text-4xl font-semibold mb-5">Favorite</h2>
